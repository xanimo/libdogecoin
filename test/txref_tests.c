...
void test_txref()
{
    ...
    for (i = 0; i < sizeof(test_vectors) / sizeof(test_vectors[0]); ++i) {
        ...
        if (test_vectors[i].enc_fail != 3) {
            res = dogecoin_txref_encode(encoded_txref, test_vectors[i].hrp, test_vectors[i].magic, test_vectors[i].blockheight, test_vectors[i].pos, test_vectors[i].non_std);
            ...
            res = dogecoin_txref_decode(encoded_txref, hrpbuf, &magic, &bh_check, &pos_check);
        }
        else {
            res = dogecoin_txref_decode(test_vectors[i].txref_encoded, hrpbuf, &magic, &bh_check, &pos_check);
        }
