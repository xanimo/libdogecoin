...
#include <dogecoin/build_assert.h>
...
#ifndef SWIG
struct secp256k1_context_struct *dogecoin_get_secp_context(void)
{
    return (struct secp256k1_context_struct *)secp_ctx();
}
#endif

int dogecoin_secp_randomize(const unsigned char *bytes, size_t bytes_len)
{
    secp256k1_context *ctx;

    if (!bytes || bytes_len != DOGECOIN_SECP_RANDOMIZE_LEN)
...
int dogecoin_free_string(char *str)
{
    if (!str)
        return DOGECOIN_EINVAL;
    dogecoin_clear(str, strlen(str));
    dogecoin_free(str);
    return DOGECOIN_OK;
}

int dogecoin_bzero(void *bytes, size_t len)
{
    if (!bytes)
        return DOGECOIN_EINVAL;
    dogecoin_clear(bytes, len);
    return DOGECOIN_OK;
}

int dogecoin_bip340_tagged_hash(const unsigned char *bytes, size_t bytes_len,
                             const char *tag, unsigned char *bytes_out, size_t len)
{
    ...
    dogecoin_clear_2(&sha, sizeof(sha), &ctx, sizeof(ctx));
    return DOGECOIN_OK;
}

int dogecoin_sha256(const unsigned char *bytes, size_t bytes_len,
...
int dogecoin_sha256_midstate(const unsigned char *bytes, size_t bytes_len,
                          unsigned char *bytes_out, size_t len)
{
    struct sha256 sha;
    struct sha256_ctx ctx;
...
int dogecoin_sha256d(const unsigned char *bytes, size_t bytes_len,
                  unsigned char *bytes_out, size_t len)
{
    ...
    if (!aligned) {
        ...
        dogecoin_clear(&sha_2, sizeof(sha_2));
    }
    dogecoin_clear(&sha_1, sizeof(sha_1));
    return DOGECOIN_OK;
}

int dogecoin_sha512(const unsigned char *bytes, size_t bytes_len,
                 unsigned char *bytes_out, size_t len)
{
    struct sha512 sha;
...
int dogecoin_ripemd160(const unsigned char *bytes, size_t bytes_len,
                    unsigned char *bytes_out, size_t len)
{
    ...
    if (!aligned) {
        ...
        dogecoin_clear(&ripemd, sizeof(ripemd));
    }
    return DOGECOIN_OK;
}

int dogecoin_hash160(const unsigned char *bytes, size_t bytes_len,
                  unsigned char *bytes_out, size_t len)
{
    ...
    if (dogecoin_sha256(bytes, bytes_len, buff, sizeof(buff)) != DOGECOIN_OK)
        return DOGECOIN_EINVAL;
    ...
    dogecoin_clear(&buff, sizeof(buff));
    return DOGECOIN_OK;
}
 ...
 */
static void dogecoin_internal_bzero(void *dest, size_t len)
{
#ifdef _WIN32
...
static void *dogecoin_internal_malloc(size_t size)
{
    return malloc(size);
}

static void dogecoin_internal_free(void *ptr)
{
    if (ptr)
        free(ptr);
}

static int dogecoin_internal_ec_nonce_fn(unsigned char *nonce32,
...
struct secp256k1_context_struct *dogecoin_get_new_secp_context(void)
{
    return secp256k1_context_create(SECP256K1_CONTEXT_VERIFY | SECP256K1_CONTEXT_SIGN);
}

struct secp256k1_context_struct *dogecoin_internal_secp_context(void)
{
    ...
    if (!global_ctx)
        global_ctx = dogecoin_get_new_secp_context();

    return global_ctx;
}

static struct dogecoin_operations _ops = {
    sizeof(struct dogecoin_operations),
    dogecoin_internal_malloc,
    dogecoin_internal_free,
    dogecoin_internal_bzero,
    dogecoin_internal_ec_nonce_fn,
    dogecoin_internal_secp_context,
    NULL,
    NULL,
    NULL,
    NULL
...
void *libdogecoin_calloc(size_t size)
{
    void *p = _ops.malloc_fn(size);
    (void) dogecoin_bzero(p, size);
    return p;
}
...
char *dogecoin_strdup(const char *str)
{
    size_t len = strlen(str) + 1;
    char *new_str = (char *)dogecoin_malloc(len);
    if (new_str)
        memcpy(new_str, str, len); /* Copies terminating nul */
    return new_str;
}

const struct dogecoin_operations *dogecoin_ops(void)
{
    return &_ops;
}

int dogecoin_get_operations(struct dogecoin_operations *output)
{
    if (!output || output->struct_size != sizeof(struct dogecoin_operations))
        return DOGECOIN_EINVAL;
    memcpy(output, &_ops, sizeof(_ops));
    return DOGECOIN_OK;
}

int dogecoin_set_operations(const struct dogecoin_operations *ops)
{
    if (!ops || ops->struct_size != sizeof(struct dogecoin_operations))
        return DOGECOIN_EINVAL; /* Null or invalid version of ops */
...
int dogecoin_is_elements_build(size_t *written)
{
    if (!written)
        return DOGECOIN_EINVAL;
...
void dogecoin_clear(void *p, size_t len){
    _ops.bzero_fn(p, len);
}

void dogecoin_clear_2(void *p, size_t len, void *p2, size_t len2){
    _ops.bzero_fn(p, len);
    _ops.bzero_fn(p2, len2);
}

void dogecoin_clear_3(void *p, size_t len, void *p2, size_t len2,
                   void *p3, size_t len3){
    _ops.bzero_fn(p, len);
    _ops.bzero_fn(p2, len2);
...
void dogecoin_clear_4(void *p, size_t len, void *p2, size_t len2,
...
static bool dogecoin_init_done = false;

int dogecoin_init(uint32_t flags)
{
    if (flags)
        return DOGECOIN_EINVAL;

    if (!dogecoin_init_done) {
        sha256_optimize();
        dogecoin_init_done = true;
    }

    return DOGECOIN_OK;
}

int dogecoin_cleanup(uint32_t flags)
{
    if (flags)
        return DOGECOIN_EINVAL;
    if (global_ctx) {
        dogecoin_secp_context_free(global_ctx);
        global_ctx = NULL;
    }
    return DOGECOIN_OK;
}

void dogecoin_secp_context_free(struct secp256k1_context_struct *ctx)
{
...
{
    unsigned char *p = dogecoin_malloc(new_n * size);
    if (!p)
        return NULL;
    if (src)
        memcpy(p, src, old_n * size);
    dogecoin_clear(p + old_n * size, (new_n - old_n) * size);
    return p;
}
...
#ifdef __ANDROID__
#define malloc(size) dogecoin_malloc(size)
#define free(ptr) dogecoin_free(ptr)
