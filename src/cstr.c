...
static int cstr_alloc_min_sz(cstring* s, size_t sz) {
    ...
    char* new_s;
    sz++; /* NULL overhead */
    if (s->alloc && (s->alloc >= sz)) return 1;
    shift = 3;
...
int cstr_alloc_minsize(cstring* s, size_t new_sz) {
    if (new_sz == s->len) return 1; /* no change */
    if (new_sz <= s->len) return 0; /* truncate string */
    if (!cstr_alloc_min_sz(s, new_sz)) return 0; /* increase string size */
    s->str[s->len] = 0; /* contents of string tail undefined */
    return 1;
}
...
cstring* cstr_new(const char* init_str) {
    size_t slen;
    if (!init_str || !*init_str) return cstr_new_sz(0);
...
void cstr_free(cstring* s, int free_buf) {
    if (!s) return;
    if (free_buf) dogecoin_free(s->str);
    memset(s, 0, sizeof(*s));
    dogecoin_free(s);
}
...
int cstr_append_buf(cstring* s, const void* buf, size_t sz) {
    if (!cstr_alloc_min_sz(s, s->len + sz)) return 0;
...
int cstr_erase(cstring* s, size_t pos, ssize_t len) {
    ssize_t old_tail;
    if (pos == s->len && len == 0) return 1;
    if (pos >= s->len) return 0;
    old_tail = s->len - pos;
    if ((len >= 0) && (len > old_tail)) return 0;
