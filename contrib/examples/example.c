...
//  for windows, use cl.exe (Visual Studio command line compiler) and link to the dogecoin.lib file and windows libraries:
//  run C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvars64.bat first to set up the environment.
//  cl.exe contrib/examples/example.c /I"include\dogecoin" /link "build\Debug\dogecoin.lib" ncrypt.lib tbs.lib msvcrt.lib advapi32.lib /out:example.exe
