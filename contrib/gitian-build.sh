...
macosSdkUrl="https://bitcoincore.org/depends-sources/sdks/Xcode-12.1-12A7403-extracted-SDK-with-libcxx-headers.tar.gz"
macosSdkHash="be17f48fd0b08fb4dcd229f55a6ae48d9f781d210839b4ea313ef17dd12d6ea5"
...
SIGNER=
VERSION=
url=https://github.com/xanimo/libdogecoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of libdogecoin. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/xanimo/libdogecoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/libdogecoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/xanimo/gitian.sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/libdogecoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean libdogecoin git directory because of old caching
    if [ -d inputs/libdogecoin/ ]; then
        echo "Cleaning libdogecoin directory..."
        rm -rf inputs/libdogecoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit libdogecoin="$COMMIT" --url libdogecoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
