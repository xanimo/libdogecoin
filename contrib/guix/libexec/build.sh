...
case "$HOST" in
    *darwin*)
        ...
        # x86_64-apple-darwin11-ld from cctools
        ;;
        ...
        case "$HOST" in
            ...
            *darwin*)
                find "${DISTNAME}" -print0 \
                    | sort --zero-terminated \
                    | tar --create --no-recursion --mode='u+rw,go+r-w,a+X' --null --files-from=- \
                    | gzip -9n > "${OUTDIR}/${DISTNAME}-${HOST//x86_64-apple-darwin11/osx64}.tar.gz" \
                    || ( rm -f "${OUTDIR}/${DISTNAME}-${HOST//x86_64-apple-darwin11/osx64}.tar.gz" && exit 1 )
                ;;
        esac
    )
)
