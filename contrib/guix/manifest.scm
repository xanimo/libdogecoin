...
(packages->manifest
 (append
  ...
  ;; guix's cross-base rejects the '-pc-' form gitian uses for the 32- and
  ;; 64-bit linux hosts, so normalise those two before asking it for a
  ;; toolchain. HOST itself stays as gitian spells it: depends, build.sh and
  ;; pack.sh all name their outputs after it, and pack.sh puts it in the
  ;; directory name inside each archive. The i686_linux_*/x86_64_linux_*
  ;; overrides in libexec/build.sh point depends at the guix-named tools; those
  ;; variable prefixes come from $(host_arch)_$(host_os), which is identical for
  ;; both spellings.
  (let ((target (let ((host (getenv "HOST")))
                  (cond ((string=? host "i686-pc-linux-gnu")   "i686-linux-gnu")
                        ((string=? host "x86_64-pc-linux-gnu") "x86_64-linux-gnu")
                        (else host)))))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "darwin")
           ;; clang-toolchain-10 is LLVM 10.0.1, the same release
           ;; depends/packages/native_clang.mk downloads. FORCE_USE_SYSTEM_CLANG
           ;; makes depends skip that download and use this clang instead, so
           ;; darwin_native_packages builds only native_cctools and
           ;; native_libtapi.
           ;;
           ;; No libc++ package is needed: the Xcode 12.2 SDK that OSX_SDK
           ;; points at ships its own libc++ headers.
           ;;
           ;; cmake is for native_libtapi, whose build.sh drives cmake.
           (list clang-toolchain-10 binutils cmake))
          (else '())))))
