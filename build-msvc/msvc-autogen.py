#!/usr/bin/env python3
# Copyright (c) 2016-2021 The dogecoin Core developers
...
def parse_makefile(makefile):
    with open(makefile, 'r', encoding='utf-8') as file:
        ...
        for line in file.read().splitlines():
            if current_lib:
                source = line.split()[0]
                if source.endswith('.c') and not source.startswith('$') and source not in ignore_list:
...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_VERSION"] = f"\"{config_dict['_PKG_VERSION_MAJOR']}.{config_dict['_PKG_VERSION_MINOR']}.{config_dict['_PKG_VERSION_BUILD']}\""
    version = config_dict["PACKAGE_VERSION"].strip('"')
    config_dict["PACKAGE_STRING"] = f"\"libdogecoin {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/libdogecoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/libdogecoin_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='libdogecoin msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/libdogecoin_config.h'), os.path.join(SOURCE_DIR, 'config/libdogecoin-config.h'))
