#ifndef __LIBDOGECOIN_CRYPTO_H__
#define __LIBDOGECOIN_CRYPTO_H__
 ...
 */
LIBDOGECOIN_API int dogecoin_scrypt(
 ...
 */
LIBDOGECOIN_API int dogecoin_aes(
 ...
 */
LIBDOGECOIN_API int dogecoin_aes_cbc(
    const unsigned char *key,
    size_t key_len,
...
/** Output length for `dogecoin_sha256` */
#define SHA256_LEN 32

/** Output length for `dogecoin_sha512` */
 ...
 */
LIBDOGECOIN_API int dogecoin_sha256(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 * FIXED_SIZED_OUTPUT(len, bytes_out, SHA256_LEN)
 */
LIBDOGECOIN_API int dogecoin_sha256_midstate(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 * FIXED_SIZED_OUTPUT(len, bytes_out, SHA256_LEN)
 */
LIBDOGECOIN_API int dogecoin_sha256d(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 */
LIBDOGECOIN_API int dogecoin_sha512(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 * FIXED_SIZED_OUTPUT(len, bytes_out, SHA256_LEN)
 */
LIBDOGECOIN_API int dogecoin_bip340_tagged_hash(
    const unsigned char *bytes,
    size_t bytes_len,
    const char *tag,
    unsigned char *bytes_out,
...
/** Output length for `dogecoin_ripemd160` */
 ...
 */
LIBDOGECOIN_API int dogecoin_ripemd160(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);

/** Output length for `dogecoin_hash160` */
 ...
 */
LIBDOGECOIN_API int dogecoin_hash160(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);
...
/** Output length for `dogecoin_hmac_sha256` */
#define HMAC_SHA256_LEN 32

/** Output length for `dogecoin_hmac_sha512` */
 ...
 */
LIBDOGECOIN_API int dogecoin_hmac_sha256(
    const unsigned char *key,
    size_t key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_hmac_sha512(
    const unsigned char *key,
    size_t key_len,
    const unsigned char *bytes,
    size_t bytes_len,
...
/** Output length for `dogecoin_pbkdf2_hmac_sha256` */
#define PBKDF2_HMAC_SHA256_LEN 32

/** Output length for `dogecoin_pbkdf2_hmac_sha512` */
 ...
 */
LIBDOGECOIN_API int dogecoin_pbkdf2_hmac_sha256(
    const unsigned char *pass,
    size_t pass_len,
    const unsigned char *salt,
    size_t salt_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_pbkdf2_hmac_sha512(
    const unsigned char *pass,
    size_t pass_len,
    const unsigned char *salt,
    size_t salt_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_private_key_verify(
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_public_key_verify(
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_xonly_public_key_verify(
    const unsigned char *pub_key,
    size_t pub_key_len);
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_public_key_from_private_key(
    const unsigned char *priv_key,
    size_t priv_key_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_public_key_decompress(
    const unsigned char *pub_key,
    size_t pub_key_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 * FIXED_SIZED_OUTPUT(len, bytes_out, EC_PUBLIC_KEY_LEN)
 */
LIBDOGECOIN_API int dogecoin_ec_public_key_negate(
    const unsigned char *pub_key,
    size_t pub_key_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_public_key_bip341_tweak(
    const unsigned char *pub_key,
    size_t pub_key_len,
 ...
 *
 * See `dogecoin_ec_public_key_bip341_tweak`.
 */
LIBDOGECOIN_API int dogecoin_ec_private_key_bip341_tweak(
    const unsigned char *priv_key,
    size_t priv_key_len,
    const unsigned char *merkle_root,
    size_t merkle_root_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_sig_from_bytes_len(
    const unsigned char *priv_key,
    size_t priv_key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 *
 * Equivalent to calling `dogecoin_ec_sig_from_bytes_aux` with ``aux_rand`` set to NULL.
 */
LIBDOGECOIN_API int dogecoin_ec_sig_from_bytes(
    const unsigned char *priv_key,
    size_t priv_key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 * :param bytes: The message hash to sign.
 * :param bytes_len: The length of ``bytes`` in bytes. Must be `EC_MESSAGE_HASH_LEN`.
 * :param aux_rand: Optional auxiliary data or NULL. See `dogecoin_ec_sig_from_bytes_aux`.
 * :param aux_rand_len: The length of ``aux_rand`` in bytes. See `dogecoin_ec_sig_from_bytes_aux`.
 * :param flags: :ref:`ec-flags` indicating desired behavior.
 * :param written: Destination for the expected length of the signature, either
 *|    `EC_SIGNATURE_LEN` or `EC_SIGNATURE_RECOVERABLE_LEN`.
 */
LIBDOGECOIN_API int dogecoin_ec_sig_from_bytes_aux_len(
    const unsigned char *priv_key,
    size_t priv_key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 *|    if flags includes `EC_FLAG_RECOVERABLE`, otherwise `EC_SIGNATURE_LEN`.
 */
LIBDOGECOIN_API int dogecoin_ec_sig_from_bytes_aux(
    const unsigned char *priv_key,
    size_t priv_key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_sig_normalize(
 ...
 * :param written: Destination for the number of bytes written to ``bytes_out``.
 */
LIBDOGECOIN_API int dogecoin_ec_sig_to_der(
    const unsigned char *sig,
    size_t sig_len,
    unsigned char *bytes_out,
    size_t len,
 ...
 * FIXED_SIZED_OUTPUT(len, bytes_out, EC_SIGNATURE_LEN)
 */
LIBDOGECOIN_API int dogecoin_ec_sig_from_der(
    const unsigned char *bytes,
    size_t bytes_len,
    unsigned char *bytes_out,
    size_t len);
 ...
 * :param sig_len: The length of ``sig`` in bytes. Must be `EC_SIGNATURE_LEN`.
 */
LIBDOGECOIN_API int dogecoin_ec_sig_verify(
    const unsigned char *pub_key,
    size_t pub_key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_sig_to_public_key(
    const unsigned char *bytes,
    size_t bytes_len,
    const unsigned char *sig,
    size_t sig_len,
 ...
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_verify(
 ...
 *
 * .. note:: Computes (scalar + operand) % n. Returns `DOGECOIN_ERROR` if
 *|    either input is not within the secp256k1 group order n.
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_add(
    const unsigned char *scalar,
    size_t scalar_len,
 ...
 *|    either input is not within the secp256k1 group order n.
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_subtract(
    const unsigned char *scalar,
    size_t scalar_len,
    const unsigned char *operand,
    size_t operand_len,
 ...
 *|    either input is not within the secp256k1 group order n.
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_multiply(
    const unsigned char *scalar,
    size_t scalar_len,
    const unsigned char *operand,
    size_t operand_len,
 ...
 *
 * .. note:: As per `dogecoin_ec_scalar_add` with ``scalar`` modified in place.
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_add_to(
 ...
 *
 * .. note:: As per `dogecoin_ec_scalar_subtract` with ``scalar`` modified in place.
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_subtract_from(
    unsigned char *scalar,
    size_t scalar_len,
    const unsigned char *operand,
    size_t operand_len);
 ...
 *
 * .. note:: As per `dogecoin_ec_scalar_multiply` with ``scalar`` modified in place.
 */
LIBDOGECOIN_API int dogecoin_ec_scalar_multiply_by(
    unsigned char *scalar,
    size_t scalar_len,
    const unsigned char *operand,
    size_t operand_len);
 ...
 * :param written: Destination for the number of bytes written to ``bytes_out``.
 */
LIBDOGECOIN_API int dogecoin_format_bitcoin_message(
    const unsigned char *bytes,
    size_t bytes_len,
    uint32_t flags,
    unsigned char *bytes_out,
 ...
 * FIXED_SIZED_OUTPUT(len, bytes_out, SHA256_LEN)
 *
 * .. note:: If ``priv_key`` is invalid, this call returns `DOGECOIN_ERROR`.
 */
LIBDOGECOIN_API int dogecoin_ecdh(
    const unsigned char *pub_key,
    size_t pub_key_len,
    const unsigned char *priv_key,
    size_t priv_key_len,
...
#define DOGECOIN_S2C_DATA_LEN 32
/** The length of a sign-to-contract (s2c) opening */
#define DOGECOIN_S2C_OPENING_LEN 33
 ...
 * :param s2c_data_len: The length of ``s2c_data`` in bytes. Must be `DOGECOIN_S2C_DATA_LEN`.
 * :param flags: Must be `EC_FLAG_ECDSA`.
 * :param s2c_opening_out: Destination for the resulting opening information.
 * FIXED_SIZED_OUTPUT(s2c_opening_out_len, s2c_opening_out, DOGECOIN_S2C_OPENING_LEN)
 * :param bytes_out: Destination for the resulting compact signature.
 * FIXED_SIZED_OUTPUT(len, bytes_out, EC_SIGNATURE_LEN)
 */
LIBDOGECOIN_API int dogecoin_s2c_sig_from_bytes(
    const unsigned char *priv_key,
    size_t priv_key_len,
    const unsigned char *bytes,
    size_t bytes_len,
 ...
 *|    `DOGECOIN_S2C_OPENING_LEN`.
 * :param flags: Must be `EC_FLAG_ECDSA`.
 */
LIBDOGECOIN_API int dogecoin_s2c_commitment_verify(
    const unsigned char *sig,
    size_t sig_len,
    const unsigned char *s2c_data,
    size_t s2c_data_len,
...
#endif /* __LIBDOGECOIN_CRYPTO_H__ */
