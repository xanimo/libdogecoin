#ifndef __LIBDOGECOIN_INTERNAL_H__
#define __LIBDOGECOIN_INTERNAL_H__
...
// #include "src/secp256k1/include/secp256k1_ecdsa_s2c.h"
#endif
...
#define malloc(size) __use_dogecoin_malloc_internally__
#define calloc(size) __use_libdogecoin_calloc_internally__
#define free(ptr) __use_dogecoin_free_internally__
#ifdef strdup
#undef strdup
#endif
#define strdup(ptr) __use_dogecoin_strdup_internally__
...
#define OUTPUT_CHECK if (!output) return DOGECOIN_EINVAL; else *output = NULL
#define OUTPUT_ALLOC(typ) \
    *output = libdogecoin_calloc(sizeof(typ)); \
    if (!*output) return DOGECOIN_ENOMEM
...
const struct dogecoin_map_item *map_find_equal_integer(const struct dogecoin_map *lhs,
                                                    ...
                                                    uint32_t key);
#endif /* __LIBDOGECOIN_INTERNAL_H__ */
